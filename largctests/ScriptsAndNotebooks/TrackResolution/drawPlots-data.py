...
maxy = 0.20
if "du" in ylab or "p2" in ylab or "p3" in ylab or "phi" in ylab or "theta" in ylab: maxy = 0.1

algo = "KalmanTrack"
split = "Half10"
#
#sample1 = "mc_mcc8.8"
sample1 = "mc_mcc8.6_bnb_halfcuts3"
sample1 = sample2
#sample2 = "mc_mcc8.8_halfcuts3"
#sample2 = "data_mcc8.8"
#
#sample1 = "mc_mcc8.8_exit"
#sample2 = "data_mcc8.8_exit"
#
#sample1 = "mc_mcc8.8_exit_ntkvtx-uy-y-x-uz_bnb"
#sample2 = "mc_mcc8.8_exit_ntkvtx-uy-y-x-uz_all"
#
#sample1 = "mc_mcc8.8_contain"
#sample2 = "data_mcc8.8_contain"
#
#sample1 = "mc_mcc8.8_contain_ntkvtx-x-y-uy_bnb"
#sample2 = "mc_mcc8.8_contain_ntkvtx-x-y-uy_all"
#
...
if blind:
    ...
    h1m = file1.Get("h12"+plot).Clone("tmp1")
    htm = file1.Get("h12"+plot).Clone("tmp2")
...
for i in range (0,hmc.GetNbinsX()):
    ...
    hmcpi.Rebin(nrebin)
    if hmcpi.GetEntries() < minentries: continue
    hmcpi.Draw()
    if fitdouble:
        ...
        hmcpi.Fit("gaus","","",hmcpi.GetMean()-hmcpi.GetRMS()*1.5,hmcpi.GetMean()+hmcpi.GetRMS()*1.5)
        if savefits: c.SaveAs("mcbin"+str(i+1)+"_"+tag+"_"+split+"_"+plot+".png")
...
for i in range (0,h1m.GetNbinsX()):
    ...
    h1mpi.Rebin(nrebin)
    if h1mpi.GetEntries() < minentries: continue
    h1mpi.Draw()
    if fitdouble:
        ...
        h1mpi.Fit("gaus","","",h1mpi.GetMean()-h1mpi.GetRMS()*1.5,h1mpi.GetMean()+h1mpi.GetRMS()*1.5)
        if savefits: c.SaveAs("m1bin"+str(i+1)+"_"+tag+"_"+split+"_"+plot+".png")
...
for i in range (0,htm.GetNbinsX()):
    ...
    htmpi.Rebin(nrebin)
    if htmpi.GetEntries() < minentries: continue
    htmpi.Draw()
    if fitdouble:
        ...
        htmpi.Fit("gaus","","",htmpi.GetMean()-htmpi.GetRMS()*1.5,htmpi.GetMean()+htmpi.GetRMS()*1.5)
        if savefits: c.SaveAs("mtbin"+str(i+1)+"_"+tag+"_"+split+"_"+plot+".png")
...
for i in range (0,hdd.GetNbinsX()):
    ...
    hddpi.Rebin(nrebin)
    if hddpi.GetEntries() < minentries: continue
    hddpi.Draw()
    if fitdouble:
        ...
        hddpi.Fit("gaus","","",hddpi.GetMean()-hddpi.GetRMS()*1.5,hddpi.GetMean()+hddpi.GetRMS()*1.5)
        if savefits: c.SaveAs("ddbin"+str(i+1)+"_"+tag+"_"+split+"_"+plot+".png")
...
leg.SetNColumns(4)
if blind:
    leg.AddEntry(hmcp,"split 1,2 MC","L")
    leg.AddEntry(h1mp,"split MC","L")
    leg.AddEntry(htmp,"full MC","L")
if not blind: 
    leg.AddEntry(hmcp,sample1,"L")
    leg.AddEntry(hddp,sample2,"L")
leg.Draw()

if blind:
    c.SaveAs("blind_"+tag+"_"+split+"_"+plot+".png")
else:
